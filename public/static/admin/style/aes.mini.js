 ...
 * @constructor
 */
function Client(url,app_id,key) {
    this.app_id=app_id;
    this.key=key;
    this.send=function (params,success){
        var xhr = new XMLHttpRequest();
        xhr.open('post', url, true);
        // Listen to the upload progress.
        var key = CryptoJS.enc.Utf8.parse("07bad5311d5108d4");
        ...
        const header=CryptoJS.enc.Utf8.parse(JSON.stringify({
            app_id:this.app_id,
            time:app.time,
            sign:sign
        }));
        ...
        xhr.setRequestHeader('sign',CryptoJS.AES.encrypt(header, key, { iv: iv,mode:CryptoJS.mode.CBC,padding: CryptoJS.pad.Pkcs7}));
        var data=CryptoJS.enc.Utf8.parse(JSON.stringify(params));
        var encrypted = CryptoJS.AES.encrypt(data, CryptoJS.enc.Utf8.parse(sign.substr(0,16)), { iv: CryptoJS.enc.Utf8.parse(sign.substr(16,16)),mode:CryptoJS.mode.CBC,padding: CryptoJS.pad.Pkcs7});
        xhr.send(encrypted);
    };
}